<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
  ...
  <div x-show="open" class="sm:hidden" id="mobile-menu">
    <div class="pt-2 pb-3 space-y-1">
      <x-nav-link href="{{ route('register') }}" class="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-50">
        {{ __('Registrar') }}
      </x-nav-link>
      <x-nav-link href="{{ route('login') }}" class="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-50">
        {{ __('Iniciar sesión') }}
      </x-nav-link>
    </div>
  </div>
